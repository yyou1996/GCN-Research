...
parser.add_argument('--data', type=str, default='~/data_zzy', help='location of the data corpus')
...
parser.add_argument('--save_dir', help='The directory used to save the trained models', default='~/models_zzy', type=str)
...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size * 8, shuffle=True,
    ...
    for epoch in range(args.epochs):
        ...
        prec1 = validate(val_loader, model, criterion, criterion_graph)
...
def train(train_loader, model, criterion, criterion_graph, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, graph_pre, graph_gt, _ = model([input,target])
...
# 
        ...
        '''
        if epoch <= 20:
            loss2 = criterion_graph(graph_pre, graph_gt)
        else:
            loss2 = 0
        '''
        loss2 = criterion_graph(graph_pre, graph_gt)
...
def validate(val_loader, model, criterion, criterion_graph):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if input.size()[0] == 1024:

            input = input.cuda()
            target = target.cuda()

            with torch.no_grad():
                output,g1,g2,gp = model([input,target], graph_ensemble=False)
            psu_target = torch.argmax(output, dim=1)
            argst = torch.argsort(psu_target)
            inputs = input[argst].view(8, 128, 3, 32, 32)
            targets = target[argst].view(8, -1)

            for n in range(8):

                input = inputs[n].cuda()
                target = targets[n].cuda()
                # compute output
                with torch.no_grad():
                    output,g1,g2,gp = model([input,target])
                    # output = model([input,target])
                    loss = criterion(output, target)
                    loss_g = criterion_graph(g1, g2)
    ...
    print('graph loss: ', loss_g.data, gp.sum() / 2 / 128)
    # aacc = new_acc(g1,g2)
    # print(aacc)
